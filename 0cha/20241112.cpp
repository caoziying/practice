...
void solve() {
    ...
    n = high.size();
    vector<vector<ll>> f(n, vector<ll>(10, -1));
    auto dfs = [&](auto &&dfs, int i, int isLow, int isHigh, int isNum, int num) -> ll {
        if(!isLow && !isHigh && isNum && ~f[i][num]) return f[i][num];
        ll res = 0;
        ...
        for(int k = l; k <= r; k++) {
            if(!isNum && k) res += dfs(dfs, i + 1, isLow && (low[i] - '0' == k), isHigh && (high[i] - '0' == k), true, k);
            else res += dfs(dfs, i + 1, isLow && low[i] - '0' == k, isHigh && high[i] - '0' == k, (!isNum && k == 0) ? false : true, num);
        }
        if(!isLow && !isHigh && isNum) f[i][num] = res;
        return res;
    };
    cout << dfs(dfs, 0, true, true, false, 0) << endl;
    // ############################################
    // cout << res << endl;
}
