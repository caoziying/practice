...
string low = "", high = "";
vector<vector<int>> f;
int dfs(int i, int st, int isZero, int isLow, int isHigh) {
    if(i == n) {
        return !isZero;
    }
    if(!isZero && !isLow && !isHigh && f[i][st] != -1) return f[i][st]; 
    int l = isLow ? low[i] - '0' : 0;
    int r = isHigh ? high[i] - '0' : 9;
    ll res = 0;
    for(int d = l; d <= r; d ++) {
        if(isZero) {
            if(d == 0) res += dfs(i + 1, st, isZero, isLow && d + '0' == low[i], isHigh && d + '0' == high[i]);
            else res += dfs(i + 1, d, false, isLow && d + '0' == low[i], isHigh && d + '0' == high[i]);
        }
        else if(abs(st - d) >= 2) res += dfs(i + 1, d, false, isLow && d + '0' == low[i], isHigh && d + '0' == high[i]);
    }
    if(st != -1 && !isLow && !isHigh) f[i][st] = res;
    return res;
}
void solve()
{
    ...
    f.resize(n, vector<int>(10, -1));
    // cout << low << endl << high << endl;
    cout << dfs(0, -1, 1, 1, 1) << endl;
    // ############################################
    // cout << res << endl;
}
